...
public class MovimientoPathfindingEnemigos : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (indCamino != -1) {
            ...
            float distancia = Vector3.Distance(transform.position, sgtePosicion);
            if (distancia < distanciaPosicionAlcanzada) {
                indCamino++;
                if (indCamino >= lisCaminoNod.Count || indCamino == rango) {
